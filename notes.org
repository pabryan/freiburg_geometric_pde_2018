...
- Concave like points are regular (i.e. not nodes) \pause
  - Topological considerations such as whether \(w = \gamma(q) - \gamma(p)\) points in or out and whether \(T_q = T_q\) or \(w\) bisects \(T_p\), \(T_q\) do not come into play at concave like points. \pause
